...
% fixes the problem with edges
% fixes the problem with s vector by Meryem, Oct 2016
...
for iFile = 1:length(files)
    ...
    d = downsample(d,round(fsn));
    if exist('aux')
        if ~isempty(aux)
            aux = downsample(aux,round(fsn));
        end
    end
    t = downsample(t,round(fsn));
